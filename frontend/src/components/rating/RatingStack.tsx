...
interface RatingStack {
    data?: { performance: number, seo: number, accessibility: number } | null
    context?: Rating | null
    options?: options
}
...
const RatingStack = ( { data, context, options: inputOptions }: RatingStack ) => {
    ...
    const getRotation = ( label: 'performance' | 'seo' | 'accessibility' ) => {
        if ( !context || !data ) {
            return 90
        }
        ...
        return clamp( percentage * 450, 360, 540 )
          ...
          <Stack direction={'row'} spacing={1}>
              <Paper sx={{ px: 1.5, py: .5, borderRadius: theme.spacing( 4 ), backgroundColor: `${getColor( data.performance )}.main` }} elevation={1}>
                  <Stack alignItems={'center'} direction={'row'}>
                      ...
                      <ArrowUpward sx={{
                          ...
                          opacity: .75,
                          transform: `rotate(${getRotation( 'performance' )}deg)`,
                      }}/>
                  </Stack>
              </Paper>
              <Paper sx={{ px: 1.5, py: .5, borderRadius: theme.spacing( 4 ), backgroundColor: `${getColor( data.seo )}.main` }} elevation={1}>
                  <Stack alignItems={'center'} direction={'row'}>
                      ...
                      <ArrowUpward sx={{ opacity: .75, transform: `rotate(${getRotation( 'seo' )}deg)` }}/>
                  </Stack>
              </Paper>
              <Paper sx={{ px: 1.5, py: .5, borderRadius: theme.spacing( 4 ), backgroundColor: `${getColor( data.accessibility )}.main` }} elevation={1}>
                  <Stack alignItems={'center'} direction={'row'}>
                      ...
                      <ArrowUpward sx={{ opacity: .75, transform: `rotate(${getRotation( 'accessibility' )}deg)` }}/>
                  </Stack>
              </Paper>
          </Stack>
        )
